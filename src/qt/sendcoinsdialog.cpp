    ...
    model(0)
{
    ...
    ui->sendButton->setIcon(QIcon());
     ...
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a TRINECHAIN address"));
#endif
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::TRINECHAINAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(TRINECHAINUnits::formatWithUnit(TRINECHAINUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
...
//    WalletModel::SendCoinsReturn sendstatus = model->sendCoins(recipients);
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(TRINECHAINUnits::formatWithUnit(TRINECHAINUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parseTRINECHAINURI(uri, &rv))
    {
        CTRINECHAINAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    ui->labelBalance->setText(TRINECHAINUnits::formatWithUnit(unit, balance));
}
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(TRINECHAINUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
// Coin Control: copy label "Quantity" to clipboard
 ...
 void SendCoinsDialog::coinControlChangeChecked(int state)
 {
     ...
     {
         if (state == Qt::Checked)
             CoinControlDialog::coinControl->destChange = CTRINECHAINAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
         else
 ...
 void SendCoinsDialog::coinControlChangeEdited(const QString & text)
 {
     if (model)
     {
         CoinControlDialog::coinControl->destChange = CTRINECHAINAddress(text.toStdString()).Get();

         // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
             ui->labelCoinControlChangeLabel->setText("");
         else if (!CTRINECHAINAddress(text.toStdString()).IsValid())
         {
             ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:red;}");
             ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid TRINECHAIN address"));
         }
         else
         {
             ...
             else
             {
                 CPubKey pubkey;
                 CKeyID keyid;
                 CTRINECHAINAddress(text.toStdString()).GetKeyID(keyid);

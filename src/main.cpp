...
const string strMessageMagic = "TRINECHAIN Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2014-04-19 Adriano https://TRINECHAINtalk.org/index.php?action=profile;u=112568
        // The following address was lost during distribution with 196780608.602771 coins in it. Blocking just in case :-)
        static const CTRINECHAINAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){  // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){  // extract the destination of the previous transaction's vout[n]
                CTRINECHAINAddress addressSource(source);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal TRINECHAIN miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TRINECHAIN
    bool fStrictPayToScriptHash = true; // Always active in TRINECHAIN
    ...
    // ppcoin: fees are not collected by miners as in TRINECHAIN
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TRINECHAIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "CBigNum vector TRINECHAIN empty mapBlockIndex TRINECHAIN TRINECHAIN";
        CTransaction txNew;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("TRINECHAINMiner : proof-of-work not meeting target");

    //// debug print
    printf("TRINECHAINMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TRINECHAINMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("TRINECHAINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadTRINECHAINMiner(void* parg);

static bool fGenerateTRINECHAINs = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void TRINECHAINMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("TRINECHAIN-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateTRINECHAINs || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateTRINECHAINs) && !fProofOfStake)
                return;
        }
        ...
        printf("Running TRINECHAINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadTRINECHAINMiner(void* parg)
{
    ...
    try
    {
        ...
        TRINECHAINMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadTRINECHAINMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadTRINECHAINMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadTRINECHAINMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateTRINECHAINs(bool fGenerate, CWallet* pwallet)
{
    fGenerateTRINECHAINs = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateTRINECHAINs = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d TRINECHAINMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadTRINECHAINMiner, pwallet))
                printf("Error: NewThread(ThreadTRINECHAINMiner) failed\n");
            Sleep(10);
        }
    }
}
